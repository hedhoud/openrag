...
Hybrid BM25:
    ...
    Because the sparse query requires the **raw query text**, hybrid search
    lives on a Milvus-specific public method, :meth:`hybrid_search`, not on
    the :class:`VectorStore` ABC (which is embedding-only).
"""
...
class MilvusVectorStore(VectorStore):
    ...
    async def search(
        ...
        legacy MilvusDB. Returns raw dicts (``id``, ``score``, plus entity
        fields except ``vector``).

        For hybrid dense + BM25 sparse, use :meth:`hybrid_search` — Milvus's
        server-side BM25 ``Function`` needs the raw query text, which the
        embedding-only ABC cannot carry.
        """
        self._resolve_collection(collection)
        ...
        try:
            response = await self._async_client.search(
                ...
                data=[embedding],
                anns_field="vector",
                search_params=DEFAULT_DENSE_SEARCH_PARAMS,
                limit=top_k,
                filter=expr,
                output_fields=["*"],
            )
    ...
    async def hybrid_search(
        ...
        Milvus-specific (not on :class:`VectorStore` ABC) because the BM25
        path requires the raw query text — the sparse vector is generated
        server-side by the schema's ``Function(FunctionType.BM25)`` over the
        ``text`` field, both at insert and at query time.

        Raises :class:`VDBSearchError` when called on a store whose backing
        collection was created with ``hybrid_search=False`` — that collection
        has no ``sparse`` field to query.
        """
        if not self._hybrid:
            raise VDBSearchError(
                f"hybrid_search called but collection `{self._collection_name}` "
                "was created with hybrid_search=False; no sparse field exists. "
        ...
        dense_req = AnnSearchRequest(
            data=[embedding],
            anns_field="vector",
            param=DEFAULT_DENSE_SEARCH_PARAMS,
            limit=top_k,
            expr=expr,
        )
