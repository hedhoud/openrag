...
class TestHybridSearchGate:
    @pytest.mark.asyncio
    async def test_hybrid_search_raises_when_hybrid_disabled(
        self,
        vdb_config: VectorDBConfig,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A store built with ``hybrid_search=False`` has no ``sparse`` field, so
        the BM25 path must refuse instead of returning empty results.
        """
        import openrag.services.storage.milvus_store as _store_mod
        ...
        with pytest.raises(VDBSearchError, match="hybrid_search=False"):
            await store.hybrid_search([0.1, 0.2], "query", collection="default")
