...
class TestEndToEnd:
    ...
    @pytest.mark.asyncio
    async def test_dense_search_returns_results(self, hybrid_store: MilvusVectorStore) -> None:
        await hybrid_store.initialize(_EMBEDDING_DIM)
        chunks = [
            _chunk("alpha", "p1", 0.1),
            _chunk("beta", "p1", 0.5),
        ...
        await hybrid_store.upsert(chunks)
        ...
        hits = await hybrid_store.search(_embedding(0.1), top_k=10)
        assert len(hits) >= 1
        for hit in hits:
            assert "id" in hit
            assert "score" in hit
    ...
    @pytest.mark.asyncio
    async def test_search_with_partition_filter(self, hybrid_store: MilvusVectorStore) -> None:
        await hybrid_store.initialize(_EMBEDDING_DIM)
        ...
        hits = await hybrid_store.search(_embedding(0.1), top_k=10, filters={"partition": "p1"})
        assert len(hits) >= 1
        for hit in hits:
...
class TestHybridSearch:
    ...
    async def test_hybrid_search_returns_fused_results(self, hybrid_store: MilvusVectorStore) -> None:
        ...
        hits = await hybrid_store.hybrid_search(
